...
static struct ListNode ** freeList = sbrk(128);
static int initializer = 1;
/*
...
void *malloc(size_t size) {
    ...
    if(initializer){
        ...
        for(int i=0;i<16;i++) freeList[i] = NULL;
        
        initializer = 0;
    }
    
    size_t index = block_index(size);
    size_t reqChunk = 1<<index;
    
   
    if(freeList[index]==NULL) {
        
        void * bigChunk = sbrk(CHUNK_SIZE);
        
        while (bigChunk!=NULL){
            
            void *tempNode = bigChunk;
            tempNode = tempNode +reqChunk;
            struct ListNode * current = freeList[index];
            freeList[index] = (struct ListNode*) tempNode;
            freeList[index]->header = reqChunk;
            freeList[index]->next = current;
            bigChunk = bigChunk + reqChunk;
        }
    ...
    if(freeList[index]!=NULL){
        ...
        chunkPtr = freeList[index];
        size_t header = freeList[index]->header;
        *(size_t*)chunkPtr = header;
        chunkPtr += sizeof(size_t);
        freeList[index] = freeList->next;
